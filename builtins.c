...
int	builtin_cd(char **args, t_exec_status *status, t_arena *env_arena)
{
	...
	if (!path)
	{
		...
		if (!path)
			return (error_handler(status, "HOME", ERR_NOT_SET, 1));
	}
	if (chdir(path) == -1)
		return (error_handler(status, "cd", ERR_OPEN, 1));
	new_pwd = getcwd(NULL, 0);
	if (!new_pwd)
		return (error_handler(status, "cd", ERR_GETCWD_FAILED, 1));
...
int	builtin_pwd(t_exec_status *status, t_arena *env_arena)
{
	...
	pwd = getcwd(NULL, 0);
	if (!pwd)
		return (error_handler(status, "pwd", ERR_GETCWD_FAILED, 1));
	ft_putstr_fd(pwd, 1);
	ft_putstr_fd("\n", 1);
	free(pwd);
	return (0);
	...
	while (args[i])
	{
		...
		if (equals)
		{
			*equals = '\0';
			...
			{
				*equals = '=';
				return (error_handler(status, "export", ERR_MEMORY_ERROR, 1));
			}
			*equals = '=';
		}
		if (!is_valid_env_name(key))
			return (error_handler(status, "export", ERR_INVALID_IDENTIFIER, 1));
		i++;
	}
	return (0);
}
...
int	builtin_unset(t_arena *env_arena, t_exec_status *status, char **args)
{
	...
	if (!args[1])
		return (error_handler(status, "unset", ERR_NOT_ENOUGH_ARGS, 1));
	i = 1;
	while (args[i])
	{
		if (!is_valid_env_name(args[i]))
		{
			error_handler(status, "unset", ERR_INVALID_IDENTIFIER, 1);
			return (1);
		}
...
int	builtin_env(t_arena *arena, t_exec_status *status)
{
	...
	if (!arena || !arena->ptrs)
		return (error_handler(status, "env", ERR_NOT_SET, 1));
	i = 0;
...
int	builtin_exit(t_ast *node, t_exec_status *status)
{
	...
	if (node->args[2])
		return (error_handler(status, "exit", ERR_TOO_MANY_ARGS, 1));
	if (!is_valid_exit_arg(node->args[1]))
		return (error_handler(status, "exit", ERR_NUMERIC_ARG_REQUIRED, 1));
