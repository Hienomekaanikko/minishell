...
int	builtin_cd(char **args, t_exec_status *status, t_arena *env_arena)
{
	...
	if (!path)
	{
		...
		if (!path)
			return (error_handler(status, "HOME", ERR_NOT_SET));
	}
	if (chdir(path) == -1)
		return (error_handler(status, "cd", ERR_OPEN));
...
int	builtin_pwd(t_exec_status *status, t_arena *env_arena)
{
	...
	pwd = getcwd(NULL, 0);
	if (!pwd)
		return (error_handler(status, "pwd", ERR_GETCWD_FAILED));
	ft_putstr_fd(pwd, 1);
	ft_putstr_fd("\n", 1);
	free(pwd);
	return (0);
	...
	while (args[i])
	{
		...
		if (equals)
		{
			*equals = '\0';
			...
			{
				*equals = '=';
				return (error_handler(status, "export", ERR_MEMORY_ERROR));
			}
			*equals = '=';
		}
