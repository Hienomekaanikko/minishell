...
/*   Updated: 2025/06/11 10:46:36 by msuokas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*find_executable(t_ast *node, t_data *data)
{
	...
	if (ft_strchr(node->value, '/'))
	{
		...
		{
			error(&data->status, node->value, NOFILE, 126);
			return (NULL);
		}
	}
	...
	if (data->mem_error)
		return (NULL);
	executable = try_path(node->value, path_env, &data->status);
	free(path_env);
	if (!executable && data->status.exit_code == 0)
		error(&data->status, node->value, NOCMD, 127);
